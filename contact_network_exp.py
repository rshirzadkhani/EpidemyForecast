...
def contact_network_exp_predict(BETA, SIGMA, GAMMA, EI_0, data, network, seeds, save_path):
    SYMPTOMATIC_RATE = 0.6
    num_seeds = len(seeds)
    e_0 = EI_0[0]
    i_0 = EI_0[1]
    ...
    for ntw in network:
        ...
        print(nx.number_connected_components(static_G))
        if temporal_G is not None:
            temporal_G = temp_G[0: t_split]# : T+1]
        print("Global Efficiency of", ntw, global_efficiency_calculater(static_G, temporal_G))
        print("Algebraic Connectivity of", ntw, algebraic_connectivity_calculator(static_G, temporal_G))      
        # print("Run "+ntw.name+" experiments...") 
...
def plot_experiments(data, network, path, fname):
    ...
    for ntw in network:
        ...
        new_infected_i = []
        cum_infected_i = []
        actives = np.load("./output/"+ path + ntw.name + "_active_cases.npy")
        # new = np.load("./output/" + path + ntw.name + "_new_cases.npy")
        cum = np.load("./output/" + path + ntw.name + "_cum_cases.npy")
    ...
    # new_cases = np.array([new_infected[0],new_infected[1], 
    #                             new_infected[2],new_infected[3]]) 
    # active_cases = np.array([active_infected[0],active_infected[1], 
    #                             active_infected[2],active_infected[3]])  
    # cum_cases = np.array([cum_infected[0],cum_infected[1], 
    #                         cum_infected[2],cum_infected[3]])
    plot_graphs(active_infected, dates, "./graph/"+ResultPath[str(data_i.name)]+"/final_figures/active_cases_"+fname, networks=graph_label) 
    plot_graphs(cum_infected, dates, "./graph/"+ResultPath[str(data_i.name)]+"/final_figures/cum_cases_"+fname, networks=graph_label) 
    # plot_graphs(new_cases, dates, "./graph/"+ResultPath[str(data_i.name)]+"/final_figures/new_cases_"+fname, networks=graph_label)
    # plot_graphs(cum_cases, dates, "./graph/"+path+"cum_cases_"+fname, networks=graph_label) 
    # plot_graphs(new_cases, dates, "./graph/"+path+"new_cases_"+fname, networks=graph_label)
...
if __name__ == "__main__":
    ...
    #                          path = read_paths[i], fname = fnames[i])
                             
    
    ###################### Plot experiments fig 2 ######################
    
    # for i, data_i in enumerate(datasets):
    #     plot_experiments(data = data_i , network = ntw_list_2, path = paths[i], fname = "fig2")                                                            
